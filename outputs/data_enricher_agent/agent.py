...
class GeneratedAgent:
    ...
    def run(self, **inputs):
        ...
        res_read_csv = TOOLS['builtin.csv_read'](path=self.ctx['inputs']['csv_path'])
        self.ctx['read_csv'] = res_read_csv
        res_enrich_first = TOOLS['builtin.http_get'](url='https://api.example.com/item/{{read_csv.rows}}')
        self.ctx['enrich_first'] = res_enrich_first
        res_validate = TOOLS['builtin.schema'](obj=self.ctx['enrich_first']['json'])
